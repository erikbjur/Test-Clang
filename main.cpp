...
int main()
{
  ...
  const unsigned int ELEMENT_INDEX_TO_FIND = 975000;
  ...
  auto iterator = std::ranges::find_if( DataStorage_vector,
                                        []( const auto& obj ) { return obj->ObjectID == ELEMENT_INDEX_TO_FIND; } );
  end_time = std::chrono::high_resolution_clock::now();
  time_in_ms = end_time - start_time;
